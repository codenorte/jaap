...
@Component({
  selector: 'app-facturas-ganaderia',
  templateUrl: './facturas-ganaderia.component.html',
  styleUrls: ['./facturas-ganaderia.component.css']
})
export class FacturasGanaderiaComponent implements OnInit {
	...
	users:Users;
	lista_usuarios;
	temp_var_usuario_ganaderia:Object = false;
	dtOptions: any = {};
	...
	*/
	...
	initiateForm(lista_detallefactura): FormGroup {
		...
		return this.formBuilder.group({
			IDDETALLEFACGANADERIA: lista_detallefactura.IDDETALLEFACGANADERIA,
			IDAGUAGANADERIA: lista_detallefactura.IDAGUAGANADERIA,
	...
	*/
	getAllUserAguaganaderia(){
    	this._aguaganaderiaService.getAllUserAguaganaderia().subscribe(
				res => {
		            if(res.code == 200){
		                console.log(res);
		                this.lista_usuarios = res.data;
		                this.temp_var_usuario_ganaderia = true;
	...
	*/
	getDetallefacturaganaderia(){
    	this._detallefacturaganaderiaService.getDetallefacturaganaderia(this.idaguaganaderia).subscribe(
				res => {
		            if(res.code == 200){
		                console.log(res);
	...
	*/
	realizarPagoFactura(array_datos){
    	...
    	this._facturasganaderiaService.realizarPagoFacturaGanaderia(array_datos).subscribe(
				res => {
		            if(res.code == 200){
		                console.log(res);
		                //crear otrospagos
		                this.numero_factura = res.facturasganaderia.NUMFACTURA;
		                console.log(this.numero_factura);
		                
		                this.verFacturaGanaderiaTicket(res.facturasganaderia.IDFACTURASGANADERIA);
		                //restablecer los arrays
		                this.array_totales = [];
		                this.array_datos = [];

		                this.toastrService.Success(res.message,res.title);
		                this.getDetallefacturaganaderia();
		            }else{
		            	console.log(res);
	...
	*/
	...
	/*
  	...
  	*/
	getuseraguaganaderiaModal(allUserAguaganderia) {
		this.searchValue = '';
		//this.rerender();
		this.modalService.open(allUserAguaganderia, {ariaLabelledBy: 'modal-basic-title', size: 'xl'}).result.then(
			(result) => {
	...
	openConfirmarPagoModal(confirmarPago) {
		...
		this.array_datos.push({
			detallefacturaganaderia: this.listafacturas.value,
	...
	*/
	...
	seleccionarUser(us){
		...
		this.idaguaganaderia=us.aguaganaderia[0].IDAGUAGANADERIA;
    ...
    */
    ...
    /*reporta factura*/
    verFacturaGanaderiaTicket(factura_id){
    	this.url_reporte='verFacturaGanaderiaTicket/'+factura_id;
